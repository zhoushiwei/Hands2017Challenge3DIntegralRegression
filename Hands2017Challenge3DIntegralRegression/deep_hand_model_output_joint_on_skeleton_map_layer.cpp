...
namespace caffe {
    ...
    void DeepHandModelOutputJointOnSkeletonMapLayer<Dtype>::LayerSetUp(
        ...
        use_raw_rgb_image_ = this->layer_param_.output_joint_on_skeleton_param().use_raw_rgb_image();
        read_from_disk_ = this->layer_param_.output_joint_on_skeleton_param().read_from_disk();
        //whether to read the raw depth image from disk
        raw_rgb_image_path_ = this->layer_param_.output_joint_on_skeleton_param().raw_rgb_image_path();
        show_gt_ = this->layer_param_.output_joint_on_skeleton_param().show_gt();

        save_path_ = this->layer_param_.output_joint_on_skeleton_param().save_path();
        save_size_ = this->layer_param_.output_joint_on_skeleton_param().save_size();


        //read the skeleton size (because the size of the skeleton map may vary?)
        skeleton_size_ = this->layer_param_.output_joint_on_skeleton_param().skeleton_size();
        
        load_skeleton_ = this->layer_param_.output_joint_on_skeleton_param().load_skeleton();

        //newly added
        dataset_name_ = this->layer_param_.output_joint_on_skeleton_param().dataset_name();
        joint_num_ = this->layer_param_.output_joint_on_skeleton_param().joint_num();
    }
    template <typename Dtype>
